...
class ConditionalBN(torch.nn.Module):
    ...
    def __init__(self, in_features, out_features):
        ...
        # self.bn = torch.nn.BatchNorm2d(out_features, affine=False)
...
class G_Unet_add_all(nn.Module):
    def __init__(self, input_nc, output_nc, nz, num_downs, ngf=64,
        ...
        unet_block = UnetBlock_with_z(input_nc, output_nc, ngf, nz, unet_block,
                                      outermost=True, norm_layer=norm_layer, nl_layer=nl_layer, upsample=upsample)
        self.model = unet_block
        self.mappnet =  MappingNetwork(z_dim=nz,w_dim=128,num_layers=2)

    def forward(self, x, z):
        z = self.mappnet(z)
        z = z.unsqueeze(1).repeat([1, 8, 1])
        z_all = z
        return self.model(x, z_all,z_idx=0)
...
class UnetBlock_with_z(nn.Module):
    ...
    def forward(self, x, z_all,z_idx):
        z_img = torch.randn([x.shape[0],1,x.shape[2],x.shape[3]], device=x.device) * self.noise_strength
        z = z_all[:,z_idx]
        if self.outermost:
            x_and_z = x #+ z_img
            x1 = self.down(x_and_z)
            x2 = self.submodule(x1, z_all,z_idx+1)
            x2 = self.up(x2)
            return x2
        elif self.innermost:
            x_and_z = x + z_img
